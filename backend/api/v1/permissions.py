...
class IsOwnerOrReadOnly(permissions.BasePermission):
    ...
    set `owner_field` where to owner of object should be.
    """
...
class IsTeaacherOrReadOnly(permissions.IsAuthenticatedOrReadOnly):
    ...
    def has_permission(self, request, view):
        return bool(
            ...
            or (request.user and request.user.is_authenticated and request.user.role == "TEACHER")
        )
...
class IsStudentrOrReadOnly(permissions.IsAuthenticatedOrReadOnly):
    ...
    def has_permission(self, request, view):
        return bool(
            request.method in permissions.SAFE_METHODS
            or (request.user and request.user.is_authenticated and request.user.role == "STUDENT")
        )
...
class CanAddLecture(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return bool(course.author == request.user or course.teachers.filter(id=request.user.id).exists())
...
class CanAddHomeWork(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return bool(lecture.teacher == request.user)
...
class CanAddReadComment(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return bool(submission.author == request.user)
