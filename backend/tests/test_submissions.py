...
def test_list_submissions_by_teacher(auth_client_teacher, homework, submission, another_submission):
    """Преподаватель курса может видеть все решения к домашнему заданию."""
    url = f'/api/v1/homeworks/{homework.id}/submissions/'
    response = auth_client_teacher.get(url)
    assert response.status_code == status.HTTP_200_OK
    assert len(response.data['results']) == 2


def test_list_submissions_by_student_forbidden(auth_client_student, homework, submission, another_submission):
    """Студент может видеть только свои."""
    url = f'/api/v1/homeworks/{homework.id}/submissions/'
    response = auth_client_student.get(url)
    assert response.status_code == status.HTTP_200_OK
...
def test_create_submission_by_student(auth_client_student, homework):
...
def test_create_submission_by_teacher_forbidden(auth_client_teacher, homework):
...
def test_retrieve_own_submission_by_student(auth_client_student, homework, submission):
...
def test_retrieve_others_submission_by_student_forbidden(auth_client_student, homework, another_submission):
...
def test_retrieve_submission_by_teacher(auth_client_teacher, homework, submission, another_submission):
...
def test_update_own_submission_by_student(auth_client_student, homework, submission):
...
def test_delete_own_submission_by_student(auth_client_student, homework, submission):
    """Студент может удалить свое решение."""
    url = f'/api/v1/homeworks/{homework.id}/submissions/{submission.id}/'
    payload = {
        "text": "updated",
    }
    response = auth_client_student.delete(url, payload)
